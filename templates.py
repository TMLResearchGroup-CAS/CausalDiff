...
def cifar100_autoenc():
	conf = ffhq64_autoenc()
	conf.data_name = 'cifar100'
	conf.eval_every_samples = 10_000_000
	conf.eval_ema_every_samples = 10_000_000
	# conf.total_samples = 72_000_000
	conf.total_samples = 100_000_000
...
def pretrain_cifar10_72M():
    conf = cifar10_autoenc()
    # conf.pretrain = PretrainConfig(
    #     name='72M',
    #     path=f'checkpoints/{cifar10_autoenc().name}/last.ckpt',
    # )
    conf.latent_infer_path = f'checkpoints/{cifar10_autoenc().name}/latent.pkl'
    return conf
