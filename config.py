...
data_paths = {
	...
	'cifar10':
	os.path.expanduser('/data/users/zhangmingkun//DiffPure-master/dataset'),
	'cifar100':
	os.path.expanduser('/data/users/zhangmingkun//DiffPure-master/dataset'),
}
...
class TrainConfig(BaseConfig):
	...
	eval_every_samples: int = 2_000_000
	eval_ema_every_samples: int = 2_000_000
	...
	# to be overridden
	name: str = ''
	lr_search: float = 5e-2
	lr_search_E: float = 3e-1
	reg_search: float = 0.
	ep_search: int = 10
	ep_search_E: int = 5
	t_N_search: int = 10
	...
	attack_type: str = 'apgd-ce'
	attack_type_train: str = 'apgd-ce'
	attack_lp_train: str = 'Linf'
	attack_eps_train: float = 8/255.
	adv_eps: float = 8/255.
	lp_norm: str = 'Linf'
	s_dim: int = 256
	...
	@property
	def logdir(self):
		# return f'{self.base_dir}/{self.name}'+'disentangle_zs_prior01_club_entropy_cos_pretrain_epoch200_norm_mask_threshold_3e_2_scale01'
		# return f'{self.base_dir}/{self.name}'+'_latent_cos_pretrain_epoch200_norm_mask_threshold_scale01_new_27500'
		# return f'{self.base_dir}/{self.name}'+'_prior01_pretrain_mask_threshold_scale01_decouple_modified'
		return f'{self.base_dir}/{self.name}'+'_CIFAR100_prior01_threshold_zeros01_pretrain'
	...
	def _make_diffusion_conf(self, T=None):
		if self.diffusion_type == 'beatgans':
			...
			return SpacedDiffusionBeatGansConfig(
				...
				fp16=self.fp16,
				prior01 = self.prior01,
				entropy_loss_by_z = self.entropy_loss_by_z,
				use_club = self.use_club,
	...
	def make_dataset(self, path=None, **kwargs):
		if self.data_name == 'ffhqlmdb256':
			...
			return CIFAR10(path=path or self.data_path,
							image_size=self.img_size,
							original_resolution=None,
							crop_d2c=True,
							**kwargs)
		elif self.data_name == 'cifar100':
			return CIFAR100(path=path or self.data_path,
							image_size=self.img_size,
							original_resolution=None,
							crop_d2c=True,
							**kwargs)
	...
	def make_model_conf(self):
		if self.model_name == ModelName.beatgans_ddpm:
			...
			self.model_conf = cls(
				...
				club_hidden_dim=self.club_hidden_dim,
				use_club=self.use_club,
				consistency=self.consistency,
				mask_threshold = self.mask_threshold,
			)
